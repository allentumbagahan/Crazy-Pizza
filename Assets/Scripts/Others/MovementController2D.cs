...
public class MovementController2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (_position != targetPos) //check for a new target
        {
        ...
        {
            pathTargeting = pathLeftToGo[0];
            Vector2 _positionTemp = new Vector2(transform.position.x, transform.position.y);
            Vector2 resTemp = pathTargeting- _positionTemp;
            directionAngle = Mathf.Atan2(resTemp.y, resTemp.x) * Mathf.Rad2Deg;
            //directionAngle = Vector2.SignedAngle(pathTargeting, _positionTemp);
            directionHorizontal = Mathf.Abs(pathTargeting.x - transform.position.x);
            directionVertical = Mathf.Abs(pathTargeting.y - transform.position.y);
            /*if(pathTargeting.x > transform.position.x ){
                direction =  "Right";
            }
            if(pathTargeting.x < transform.position.x ){
                direction =  "Left";
            ...
            if(pathTargeting.y > transform.position.y && directionVertical > directionHorizontal ){
                direction =  "Up";
            }
            if(pathTargeting.y < transform.position.y && directionVertical > directionHorizontal ){
                direction =  "Down";
            }*/
            direction = GetDirection(directionAngle);
